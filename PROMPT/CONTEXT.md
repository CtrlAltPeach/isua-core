...
- ✅ Итерация 12 (инфра БД + надёжность): 12A — БД пересоздана с ICU-локалью ru-RU
     (CREATE DATABASE … LOCALE_PROVIDER icu ICU_LOCALE 'ru-RU'); убран COLLATE-костыль
     в поиске абитуриентов → обычный Prisma `mode:"insensitive"` (регистронезависимый
     поиск по кириллице из коробки). Локально: dump → новая БД с ICU → restore → swap имён
     (старая C-locale сохранена как isua_c_backup). 12C — retry/переподключение в
     клиентской fetch-обёртке (lib/api.ts: до 3 попыток с backoff для GET/HEAD при сетевой
     ошибке и 502/503/504; POST/PUT/DELETE НЕ повторяются; сетевой сбой → ApiError status 0);
     интеграционные тесты API-роутов (/api/applicants GET — auth/where/пагинация; /api/programs
     POST — admin/валидация/дубликат) + тесты retry; всего 55 тестов (было 36). README заполнен
     (стек, запуск, ICU-БД, env, тесты, деплой). build + lint чисто.
- ✅ Итерация 13 (полировка): сидинг ОТКЛЮЧЁН по умолчанию (данные заводятся вручную;
     включить — флаг SEED_DATA=1; локальная БД обнулена); squash миграций — 8 файлов
     схлопнуты в одну 20260618030000_init (БД была накатана только локально, безопасно);
     балльные поля Float → Int (mathProfile/russian/chemistry/physics/informatics/geography/
     additionalScores/totalScore — целые, исторический Float от старой логики «среднее»);
     автоопределение таймзоны из браузера (lib/timezone detectTimezone() через Intl;
     store: флаг timezoneManual, applyAutoTimezone не перезатирает ручной выбор; app-shell
     эффект; FALLBACK_TIMEZONE=Europe/Moscow для SSR без hydration mismatch). Тесты 55→62
     (+detectTimezone/store). build + lint чисто.
...
1.1 ✅ БД с ICU/русской локалью (итер.12A): локальная БД пересоздана с
  LOCALE_PROVIDER icu / ICU_LOCALE 'ru-RU'; COLLATE-костыль убран, поиск через
  Prisma mode:"insensitive". Прод/dev-preview: создавать БД с ICU ru-RU при
  провижене (см. PROMPT/DATABASE_ENVIRONMENTS.md).

### ПРИОРИТЕТ 2: Полировка
5.1 ✅ Временные зоны (lib/timezone.ts через Intl, итер.9)
5.2 ✅ Toast вместо alert/confirm (итер.9)
5.3 ✅ Retry/переподключение при потере соединения (итер.12C, lib/api.ts)
5.4 ✅ Тесты unit (vitest, итер.9) + интеграционные API-роутов (итер.12C); всего 55
5.5 ✅ README заполнен (итер.12C)
...
- ✅ ИСПРАВЛЕНО (итер.12A): ILIKE/поиск не учитывал кириллицу из-за C-locale БД —
  БД пересоздана с ICU ru-RU, COLLATE-костыль убран, поиск через Prisma mode:"insensitive".
