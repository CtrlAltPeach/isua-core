...
при register/login (на уровне Zod-схемы). На ПУСТОЙ БД делать ничего не нужно.
Если на проде УЖЕ есть пользователи с email в разном регистре — один раз
привести существующие:

```sql
-- Сначала проверить отсутствие коллизий по регистру (должно вернуть 0 строк):
SELECT lower(email), count(*) FROM "User" GROUP BY lower(email) HAVING count(*) > 1;
-- Если коллизий нет — нормализовать:
UPDATE "User" SET email = lower(email) WHERE email <> lower(email);
```

(При коллизиях — сначала вручную разрулить дубли учёток.)
